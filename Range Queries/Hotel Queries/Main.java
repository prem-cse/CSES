...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		 ...
		 int n = sc.nextInt();
	        ...
	        while (q-->0){
	            int x = sc.nextInt();
	            int idx = seg.ceiling(a,0,n-1,x);
	            if(idx != -1) seg.update(a,idx,a[idx]-x);
	            ++idx;
	            out.print(idx+" ");
	        }
	        out.close();
	    ...
	    public static class SegmentTree
	    {
	        ...
	        public long query(long m, long mid2, int i, int l, int r) {
	            // OUTSIDE
	            if(m>r || mid2<l)
	                return Integer.MIN_VALUE; // depend on q
	            //INSIDE
	            if(m>=l && mid2<=r)
	                return seg[i];
	            // PARTIAL
	        ...
	        public void update(long[] a,int idx,long val) {
	            updateTree(a,0,a.length-1,1,idx,val);
	        }
	        ...
	        public int ceiling(long[] a,int l,int r,int x) {
	            long max = query(a,l,r);
	            if(max < x) return -1; // not found
	            while (l <= r){
	                int mid = (l+r)>>1;
	                long maxL = query(a,l,mid);
	                if (maxL >= x)
	                    r = mid-1;
	                else
	                    l = mid+1;
